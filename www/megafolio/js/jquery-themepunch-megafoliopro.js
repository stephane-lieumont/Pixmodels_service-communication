...
container.data('order',order);notOrdered(container);reOrder(container,0);rePosition(container);})},megafilter:function(filter){return this.each(function(){var container=$(this);if(container.data('nofilterinaction')!=1){container.data('nofilterinaction',1);addFilter(container,filter);reOrder(container,0);rePosition(container);setTimeout(function(){container.data('nofilterinaction',0);},1200)}else{clearInterval(container.data('nextfiltertimer'));container.data('nextfiltertimer',setInterval(function(){if(container.data('nofilterinaction')!=1){clearInterval(container.data('nextfiltertimer'));addFilter(container,filter);reOrder(container,0);rePosition(container);container.data('nofilterinaction',1);setTimeout(function(){container.data('nofilterinaction',0);},1200)}},10));}})},megaanimchange:function(anim,speed,rotate,scale){return this.each(function(){var container=$(this);var opt=container.data('opt');var filter=opt.filter;opt.filterChangeAnimation=anim;opt.filterChangeSpeed=speed;opt.filterChangeRotate=rotate;opt.filterChangeScale=scale;addFilter(container,"");addFilter(container,filter);setTimeout(function(){reOrder(container,0);rePosition(container);},2*opt.filterChangeSpeed);container.data('opt',opt);})}})
...
if(next_layout<2)next_layout=2;if(next_layout>23)next_layout=23;lastorder.push(next_layout);var element_amount=next_layout;if(next_layout==10||next_layout==14)element_amount=3;if(next_layout==11||next_layout==15)element_amount=4;if(next_layout==12||next_layout==16)element_amount=5;if(next_layout==13||next_layout==17)element_amount=6;if(next_layout==11||next_layout==12||next_layout==13||next_layout==15||next_layout==16||next_layout==17)
if(cw<840&&cw>721)element_amount=4
else
if(cw<720)element_amount=3;if(next_layout==18||next_layout==19||next_layout==20)element_amount=1;if(next_layout==21||next_layout==22||next_layout==23)element_amount=2;entries.slice(0,element_amount).each(function(i){var ent=$(this);ent.removeClass('tp-layout-first-item').removeClass('tp-layout-last-item').removeClass('very-last-item');ent.addClass('tp-ordered tp-layout');ent.data('layout',next_layout)
ent.data('child',i)
...
{try{findLastOrdered(container).addClass('very-last-item');}catch(e){}
return container;}}
...
if(cw<720)element_amount=3;if(next_layout==18||next_layout==19||next_layout==20)element_amount=1;if(next_layout==21||next_layout==22||next_layout==23)element_amount=2;var firstent=entries.length-itemtogo;entries.slice(firstent,firstent+element_amount).each(function(i){var ent=$(this);ent.removeClass('tp-layout-first-item').removeClass('tp-layout-last-item').removeClass('very-last-item');ent.addClass('tp-ordered tp-layout');ent.data('layout',next_layout)
ent.data('child',i)
if(i==0)ent.addClass("tp-layout-first-item");if(i==element_amount-1){ent.addClass("tp-layout-last-item");}
...
if(layout==18){w=Math.round(w980);w=w+(ph);var h=w;var chil=ent.data('child');xp=w*chil;lasth=h;topp=topp+h;}
if(layout==19){w=Math.round(w980);w=w+(ph);var h=w/2;var chil=ent.data('child');xp=w*chil;lasth=h;topp=topp+h;}
if(layout==20){w=Math.round(w980);w=w+(ph);var h=w/3;var chil=ent.data('child');xp=w*chil;lasth=h;topp=topp+h;}
if(layout==21){w=Math.round(w490);w=w+(ph/2);var h=w;var chil=ent.data('child');xp=w*chil;lasth=h;if(chil==1)topp=topp+h;}
if(layout==22){w=Math.round(w490);w=w+(ph/2);var h=w/2;var chil=ent.data('child');xp=w*chil;lasth=h;if(chil==1)topp=topp+h;}
if(layout==23){w=Math.round(w490);w=w+(ph/2);var h=w/3;var chil=ent.data('child');xp=w*chil;lasth=h;if(chil==1)topp=topp+h;}
...
var scal=1;var opaa=1;var rx=0;var rot=0;var orot=opt.filterChangeRotate;if(orot==undefined)rot=30;if(orot==99)orot=Math.round(Math.random()*360);var subfilters=opt.filter.split(',');var hasfilter=false;for(var u=0;u<subfilters.length;u++){if(ent.hasClass(subfilters[u])){hasfilter=true;console.log("has class");}}
if(hasfilter||opt.filter=="*"){ent.css({visibility:'visible'});if(ie||ie9){scal=1;opaa=1;}else{if(opt.filterChangeAnimation=="pagetop"||opt.filterChangeAnimation=="pagebottom"||opt.filterChangeAnimation=="pagemiddle"){rot=0;rx=0;scal=1;opaa=1;eiscal=1;eiopaa=1;eirx=0;}else{scal=1;rot=0;opaa=1;eiscal=1;eiopaa=1;eirx=0;}}}
...
container.animate({height:maxhh+"px"},"normal");}})(jQuery);